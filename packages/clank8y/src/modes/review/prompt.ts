...
const REVIEW_WORKFLOW = [
  ...
  '- If EVENT-LEVEL INSTRUCTIONS includes `report_back_to: @user`, mention that user so they are notified.',
  ...
  '- Native read-only file tools are allowed only so you can read `.clank8y/diff.txt` and `.clank8y/review-comments.md`.',
  '- Native read-only tools available in this mode are `read`, `grep`, `find`, and `ls`.',
  '- Use `grep` as the only local search tool, and only to search `.clank8y/diff.txt` or `.clank8y/review-comments.md` for patterns you are already investigating.',
